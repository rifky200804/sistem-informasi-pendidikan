...
export const AnecdoteDetailDialog = ({
  ...
  const studentName = (typeof anecdote.student === 'object' && anecdote.student ? anecdote.student.name : anecdote.student) || anecdote.studentName || "Tidak ada data murid";
  const description = anecdote.content || anecdote.description || "-";
  const teacherName = (typeof anecdote.teacher === 'object' && anecdote.teacher ? anecdote.teacher.name : anecdote.teacher) || (anecdote.teacherId ? `Guru ID: ${anecdote.teacherId}` : "-");
  ...
  return (
    <>
      ...
      {/* Full Image Preview Overlay — rendered outside parent Dialog to avoid Radix nesting issues */}
      {isPreviewOpen && (
        <div
